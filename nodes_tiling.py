...
class TileSplit(TilingNodeBase):
    """Splits Video Batches into a List of Video Batches for 22DogsNodes."""
...
class TileMerge(TilingNodeBase):
    ...
    def merge(self, tiles, tile_calc):
        ...
        print(f"[TileMerge] number of tiles received: {len(tile_list)}")
        for idx, t in enumerate(tile_list):
            print(f"[TileMerge]   tile[{idx}] shape={t.shape} dtype={t.dtype}")

        num_frames = tile_list[0].shape[0]
        print(f"[TileMerge] num_frames={num_frames}  canvas={tc['orig_h']}x{tc['orig_w']}")
        ...
        for b in range(num_frames):
            ...
            for i, layout in enumerate(tc['layouts']):
                ...
                print(f"[TileMerge]   frame b={b} tile={i} raw shape={raw_frame.shape}")

                tile_frame = self.ensure_4d_BCHW(raw_frame, device)
                print(f"[TileMerge]   frame b={b} tile={i} after ensure_4d shape={tile_frame.shape}")
